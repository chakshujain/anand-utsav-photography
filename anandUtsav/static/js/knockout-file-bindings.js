...
(function(factory) {
  ...
  function extendOptions(defaultOptions, newOptions) {
    ...
    for (var prop in defaultOptions) {
      options[prop] =
        typeof newOptions[prop] !== "undefined"
          ? newOptions[prop]
          : defaultOptions[prop];
    }
    return options;
  ...
  function addRemoveCssClass(element, cssClasses, type) {
    var cssClasses = Array.isArray(cssClasses)
      ? cssClasses
      : cssClasses.split(" ");
  ...
  ko.bindingHandlers.fileInput = {
    init: function(element, valueAccessor) {
      ...
      fileData.fileTypes =
        fileData.fileTypes || ko.observable(currentAcceptValue);
      ...
      function fillData(file, index, callback) {
        ...
        if (
          ...
          if (
            !(fileData.dataURLArray && ko.isObservable(fileData.dataURLArray))
          ) {
        ...
        fileProperties.forEach(function(property) {
          if (
            !(fileData[property] && ko.isObservable(fileData[property])) &&
            !(
              fileData[property + "Array"] &&
              ko.isObservable(fileData[property + "Array"])
            )
  ...
  ko.bindingHandlers.fileDrag = {
    update: function(element, valueAccessor, allBindingsAccessor) {
      ...
      if (!element.getAttribute("file-drag-injected")) {
        ...
        element.ondragover = element.ondragleave = element.ondrop = function(
          e
        ) {
          e.stopPropagation();
          e.preventDefault();
          ...
          if (e.type == "drop" && e.dataTransfer) {
            ...
            if (files.length) {
              for (var i = 0; i < files.length; i++) {
                if (validateDroppedFileType(fileInput, files[i]))
  ...
  ko.bindingHandlers.customFileInput = {
    init: function(element, valueAccessor, allBindingsAccessor) {
      ...
      var wrapper = addCssClass(document.createElement("span"), [
        sysOpts.wrapperClass,
        options.wrapperClass,
      ]);
      var buttonGroup = addCssClass(document.createElement("span"), [
        sysOpts.buttonGroupClass,
        options.buttonGroupClass,
      ]);
      var button = addCssClass(
      ...
      if (options.fileName) {
        ...
        buttonGroup.parentNode.insertBefore(
          addCssClass(fileNameInput, sysOpts.fileNameClass),
          buttonGroup
        );
        if (hasCssClass(buttonGroup, "btn-group")) {
          addCssClass(
            removeCssClass(buttonGroup, "btn-group"),
            "input-group-btn"
          );
        }
      } else {
        if (hasCssClass(buttonGroup, "input-group-btn")) {
          addCssClass(
            removeCssClass(buttonGroup, "input-group-btn"),
            "btn-group"
          );
        }
      }
      ...
      element.parentNode.insertBefore(
        addCssClass(document.createElement("span"), sysOpts.buttonTextClass),
        element
      );
    },
    update: function(element, valueAccessor, allBindingsAccessor) {
      ...
      buttonText.innerText = ko.utils.unwrapObservable(
        file ? options.changeButtonText : options.buttonText
      );
      if (options.fileName) {
        var fileNameInput = wrapper.querySelector("." + sysOpts.fileNameClass);
        addCssClass(
          fileNameInput,
          ko.utils.unwrapObservable(options.fileNameClass)
        );
      ...
      var clearButton = buttonGroup.querySelector(
        "." + sysOpts.clearButtonClass
      );
      if (!clearButton) {
        clearButton = addCssClass(
          document.createElement("span"),
          sysOpts.clearButtonClass
        );
      ...
      addCssClass(
        clearButton,
        ko.utils.unwrapObservable(options.clearButtonClass)
      );
